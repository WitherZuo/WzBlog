...
但问题又来了，要如何生成这样的二维码呢？当然不可能每篇文章自己手动生成一个二维码图片放到页面上，麻烦不说，一旦页面的地址发生变动那么这个二维码就over了，你还得手动再生成一个重复上述步骤；第二个方法是借助网上公开的QRCode服务的API接口，嵌入到当前页面中，但这同样有问题，一旦第三方服务被屏蔽或失效，那么也基本凉凉了；**所以最后只有一个办法，就是在页面上直接生成二维码**。  <!-- more -->

在页面上直接生成二维码，既不用自己手动生成二维码放上去，也可以规避因第三方服务提供商导致的服务失效问题。最后我决定使用j`Query`和`jquery.qrcode.js`来实现这个功能。  
